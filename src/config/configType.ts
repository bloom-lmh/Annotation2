...
export interface IClassAnnotationConfig
  extends Omit<IBaseAnnotationConfig, 'isAccessTag' | onlyFile> {
...
export interface IPropertyAnnotationConfig
  extends Omit<IBaseAnnotationConfig, 'isAuthorTag' | 'isAccessTag' | 'isVersionTag' | onlyFile> {
  isPropertyTag: boolean; // 是否是属性
  isStaticTag: boolean; // 是否是静态属性
  isTypeTag: boolean; // 是否包含类型
  isDefaultTag: boolean; // 是否包含默认值
}
export interface IInterfaceAnnotationConfig
  extends Omit<IBaseAnnotationConfig, 'isAccessTag' | onlyFile> {
  isInterfaceTag: boolean; // 是否是接口
  isExtendsTag: boolean; // 是否继承
}
export interface IEnumAnnotationConfig
  extends Omit<
    IBaseAnnotationConfig,
    'isAuthorTag' | 'isAccessTag' | 'isVersionTag' | 'isTemplateTag' | onlyFile
  > {
  isEnumTag: boolean; // 是否是枚举类型
}
export interface ITypedefAnnotationConfig
  extends Omit<
    IBaseAnnotationConfig,
    'isAuthorTag' | 'isAccessTag' | 'isVersionTag' | 'isTemplateTag' | onlyFile
  > {
...
export interface IFileAnnotationConfig
  extends Omit<
    IBaseAnnotationConfig,
    | 'isAccessTag'
    | 'isAliasTag'
