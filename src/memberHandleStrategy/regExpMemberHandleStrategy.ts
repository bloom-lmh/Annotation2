...
import { IMember, MethodType } from '../member/memberType';
...
export class RegExpMemberHandleStrategy implements MemberHandleStrategy {
  ...
  handleClass(memberDeclaration: MemberDeclaration): IMember | null {
    ...
    const regex =
      /(?<_abstract>abstract)?\s*class\s+(?<_name>\w+)\s*(?:extends\s+(?<_extend>\w+))?\s*(?:implements\s+(?<_implements>[A-Za-z, ]+))?/;
    // 匹配类声明
  ...
  handleMethod(memberDeclaration: MemberDeclaration): IMember | null {
    ...
    // 改进后的正则表达式
    const regex =
      /(?<_access>public|private|protected)?\s*(?<_static>static)?\s*(?<_async>async)?\s*(?<_name>\w+)\s*\((?<_params>.*?)\)\s*(?::\s*(?<_returnType>\w+))?(?:\s*throws\s*(?<_throws>[\w, ]+))?/;
    // 匹配
    const match = textMemberDeclaration.match(regex);
    // 不匹配返回默认成员
    if (!match || !match.groups) {
  ...
  handleProperty(memberDeclaration: MemberDeclaration): IMember | null {
    ...
    const regex =
      /(?<_access>public|private|protected)?\s*(?<_static>static)?\s*(?<_name>\w+)\s*:\s*(?<_type>\w+)\s*(?<_defaultValue>=\s*[^;]+)?/;
    // 首先尝试解析箭头函数
  ...
  handleEnum(memberDeclaration: MemberDeclaration): IMember | null {
    ...
    // 返回封装的 EnumMember 实例
    return new EnumMember().setName(_name).setEnum(true).setEnumMembers(_members);
  }
  ...
  handleTypedef(memberDeclaration: MemberDeclaration): IMember | null {
    ...
    const regex =
      /type\s+(?<name>\w+)\s*=\s*(?<type>(?:(?:[\w\s\|\&]+)|(?:Array<[\w\s\[\],<>]+>)|(?:[\w\s\[\]]+\[]))|(?:\(\s*([\w\s,<>:]+)\s*\)\s*=>\s*\{?[\w\s,]*\}?)|(?:\(\s*([\w\s,<>:]+)\s*\)\s*=>\s*[\w\s<>]+))/;
    // 匹配类型别名声明
    const match = textMemberDeclaration.match(regex);
    if (!match || !match.groups) {
    ...
    return new TypedefMember().setName(_name).setTypedef(_type);
  }

  private preHandle(memberDeclaration: MemberDeclaration): {
    textMemberDeclaration: string;
  ...
  private parseArrowFunction(textMemberDeclaration: string): MethodMember | null {
    const regex =
      /(?<_access>private|protected|public)?\s*(?<_static>static)?\s*(?<async>async)?\s*(?<_name>\w+)\s*=\s*(?<asyncKeyword>async)?\s*\((?<_params>[^)]*)\)\s*(?::\s*(?<_returnType>[^ ]+))?\s*=>\s*[^a-zA-Z0-9]*\b(?<_exceptionType>\w*)\b/;
