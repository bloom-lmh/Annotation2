...
export function activate(context: ExtensionContext) {
  ...
  // 监听项目变化,动态注入配置
  const workspaceFolderDisposable =
    vscode.workspace.onDidChangeWorkspaceFolders((event) => {
      let addProjectConfigs: {
        projectPath: string;
      ...
      if (panel) {
        if (addProjectConfigs && addProjectConfigs.length > 0) {
          panel.webview.postMessage({
            command: "addProjectConfig",
            data: addProjectConfigs,
          });
        }
        if (removeProjectPaths && removeProjectPaths.length > 0) {
          panel.webview.postMessage({
            command: "removeProjectConfig",
            data: removeProjectPaths,
          });
        }
      }
    });
  ...
  const disposable4 = vscode.commands.registerCommand("test", async () => {
    ...
    // 获取文件所属项目路径
    const projectPath = vscode.workspace.getWorkspaceFolder(editor.document.uri)
      ?.uri.fsPath;
    if (!projectPath) {
    ...
    let config = new ConfigLoader().loadConfig(
      projectPath + "/annotation.config.json"
    );
    console.log(config);
